class p2Level extends Phaser.Scene {
    ...
    crearJugador(jugTemp) {
        var newJugador = this.physics.add.sprite(jugTemp.x2, jugTemp.y2, 'jugador');
        this.x0 = jugTemp.x2;
        this.y0 = jugTemp.y2;
    ...
    crearJugadores(jugTemp) {
        var newJugador = this.physics.add.sprite(jugTemp.x2, jugTemp.y2, 'enemigo');
        newJugador.setOrigin(0.5, 0.5);
        newJugador.setScale(2.3, 2.3);
        newJugador.body.setSize(12, 15).setOffset(2, 8);
        if (jugTemp.flipped) {
    ...
    create() {
        ...
        Object.keys(coleccionables.balas2).forEach(function (id) {
            const tempBala = coleccionables.balas2[id];
        ...
        Object.keys(coleccionables.monedas2).forEach(function (id) {
            const tempMoneda = coleccionables.monedas2[id];
            ...
            if (!nivel2F) {
                self.jugadores.getChildren().forEach(function (atacante) {
                    if (info[0] === atacante.id) {
                        ...
                        bala.setPosition(atacante.x, atacante.y + 2);
    ...
    update() {
        ...
        if (this.jugador.dataMovimiento && (x !== this.jugador.dataMovimiento.x || y !== this.jugador.dataMovimiento.y)) {
            socket.emit('movJugador2', { x: this.jugador.x, y: this.jugador.y, flipped: this.jugador.flipX, anim: this.jugador.anims.getCurrentKey() });
        }
